# Script to train the Transformer-big model with PCA pruning.
# Note: Enable the PCA Pruning in the "prune" module.

# Export variables
TF_CPP_MIN_LOG_LEVEL=2 # 0:All, 1:Info, 2:Warning , 3:None
PARAM_SET=big
DATA_DIR=/datasets/wmt
MODEL_DIR=model_pca_pruned_$PARAM_SET
VOCAB_FILE=$DATA_DIR/vocab.ende.32768
NUM_GPUS=4
BATCH_SIZE=$((4096 * NUM_GPUS)) # Default Test = 4096
EPOCHS=15 # Default Test = 20
STEPS_EPOCH=20000 # Default Test = 5000
TRAIN_STEPS=$((STEPS_EPOCH * EPOCHS))
MAX_LENGTH=64 # Default Test = 64

# Train the model for 300000 steps and evaluate every 20000 steps on multiple GPUs.
