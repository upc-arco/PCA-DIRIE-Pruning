...
class TransformerTask(object):
    ...
    with distribute_utils.get_strategy_scope(self.distribution_strategy):
      ...
      if params["use_ctl"]:
        ...
        if params["enable_tensorboard"]:
          summary_writer = tf.summary.create_file_writer(os.path.join(flags_obj.model_dir, "summary"))
        else:
      ...
      if latest_checkpoint:
        ...
        logging.info("  Resuming from iteration %d", current_step)

      # Prune Model
      prune.pca_pruning(model)
      prune.uc_pruning(model)
  ...
  def eval(self):
    ...
    with distribute_utils.get_strategy_scope(distribution_strategy):
      ...
      weight_layers = [[weight_var.name, weight_var.shape] for weight_var in self.predict_model.weights]
      count_qt_layers = 0
      for idx, layer in enumerate(weight_layers):
        print("Layer: ", idx)
        print("  Name: ", layer[0])
        print("  Shape: ", layer[1])
        if ((len(layer[1]) > 1) and ("embedding" not in layer[0]) and ("mask" not in layer[0])): count_qt_layers +=1
      print("Layers to Quantize: ", count_qt_layers)
	  # PCA Pruning (Note: Disable when Training)
      #prune.pca_pruning(self.predict_model)
      prune.uc_pruning(self.predict_model)
  ...
  def _load_weights_if_possible(self, model, init_weight_path=None):
    ...
    if init_weight_path:
      ...
      if self.use_tpu:
        checkpoint = tf.train.Checkpoint(model=model, optimizer=self._create_optimizer())
...
def main(_):
  ...
  if flags_obj.mode == "train":
    ...
    task.eval()
    custom_layers.print_pca_analysis()
  else:
