# Script to test the Transformer-big model achieving a BLEU Score of 28+ while performing the PCA Analysis.
# Note: Enable the PCA in the "custom_layers" module to generate a trace.

# Export variables
TF_CPP_MIN_LOG_LEVEL=2 # 0:All, 1:Info, 2:Warning , 3:None
PARAM_SET=big
DATA_DIR=/datasets/wmt
MODEL_DIR=model_saved/baseline_$PARAM_SET
...
python3 transformer_main.py --mode=eval --data_dir=$DATA_DIR --model_dir=$MODEL_DIR \
    ...
    --bleu_source=$DATA_DIR/newstest2013.en \
    --bleu_ref=$DATA_DIR/newstest2013.de \
    --num_gpus=$NUM_GPUS \
